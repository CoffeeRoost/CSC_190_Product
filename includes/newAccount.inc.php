<?php 
    ...
    if(isset($_POST["signup-submit"])){
        ...
        $africanAmercian_black          =$_POST['africanAmercian_black'];
        $asian                          =$_POST['asian'];
        $hawaiian_other                 =$_POST['hawaiian_other'];
        $white                          =$_POST['white'];
        $noAnswer                       =$_POST['noAnswer'];

        if(empty($americanIndian_alaskanNative) && empty($africanAmercian_black) && empty($asian) && empty($hawaiian_other) && empty($white)){
            $noAnswer = 'Yes';
        }
                        ...
                        //bind info
                            ...
                            if($row = mysqli_fetch_assoc($result)){
                                ...
                                $sql5="INSERT INTO EMPLOYMENT (userID,currentMilitaryOrVet,militarySpouse,selectiveService,employmentStatus,payRate,unemployemntInsurance,unemploymentWeeks,farmworker12Months,desiredJobTitle,techExperience)
                                    VALUES(?,?,?,?,?,?,?,?,?,?,?);";
                                $sql6="INSERT INTO ETHNICITY (userID,hispanicHeritage,africanAmercian_black,americanIndian_alaskanNative,asian,hawaiian_other,white,noAnswer,primaryLanguage,englishProficiency)
                                    VALUES(?,?,?,?,?,?,?,?,?,?);";
                            ...
                            {	
                                ...
                                mysqli_stmt_bind_param($stmt6,"isssssssss",$row['userID'],$hispanic,$africanAmercian_black,$americanIndian_alaskanNative,$asian,hawaiian_other,$white,$noAnswer,$language,$proficiency);
