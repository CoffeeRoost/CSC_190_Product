...
if(isset($_POST['signup-submit'])){
  ...
  if($_POST['sameAdd'] === 'No'){
	if(empty($mailStreet)||empty($mailCity)||empty($mailCity)||empty($mailState)||empty($mailZip)||empty($mailCounty)){
		header ("Location: ../surveyTest.php?error=mandatoryMailing");
  ...
  if($gender === 'other'){
	if(empty($otherAns)){
		header("Location: ../surveyTest.php?error=mandatoryGenderDescription");
	}
  }
 ...
 ||empty($pellGrant)||empty($workTicket)||empty($homeless)||empty($exOffer)||empty($displace)||empty($singleParent)||empty($culBarrier)||empty($familySize)||empty($annualIncome)||empty($techExp)||empty($password)||empty($confirmPassword))
  {
    ...
    header ("Location: ../surveyTest.php?error=emptyfields");
    // //stop the code to run
     exit();
  }
  else if (!filter_var($email,FILTER_VALIDATE_EMAIL)){
    // check email validation
    header ("Location: ../surveyTest.php?error=invaidmail&email=".$email);
     exit();
  }
  else if ($password !== $confirmPassword){
    // check password match with confirm password
    header ("Location: ../surveyTest.php?error=passwordcheck&email=".$email);
    exit();
  }
  else
  {
    ...
    if(!mysqli_stmt_prepare($stmt,$sql)){
      header ("Location: ../surveyTest.php?error=sqlerror");
      exit();
    }
    else{
      //bind info
      ...
      if($resultCheck>0){
        header ("Location: ../surveyTest.php?error=emailtaken&email=".$email);
        exit();
      }
      else{
	    ...
	    $sql="SELECT * FROM PARTICIPATION WHERE email=?";
			...
			//check if we exactly get result from database
      		if($row = mysqli_fetch_assoc($result)){
				...
				$stmt8=mysqli_stmt_init($conn);
					...
					$stmt= mysqli_stmt_init($conn);
						...
						if(mail($to,$subject,$message,$headers)){
							...
							header("Location: ../surveyTest.php?error=emailfail");
							exit();
						}
				...
				header ("Location: ../surveyTest.php?error=nouseremail");
        			exit();
			}
		}
        }
  ...
  header ("Location: ../surveyTest.php");
  exit();
}
